package com.koodlelab.bubbletea.functionaltest

import com.koodlelab.bubbletea.BubbleTeaShopResource
import com.koodlelab.bubbletea.DrinkType
import com.koodlelab.bubbletea.Order
...
class BubbleTeaServiceTest extends Specification{

//    def 'should return closest coffee shop to Ma On Shan Center'() {
//        given:
//
//        def mongoClient = new MongoClient()
//        def bubbleTeaShop = new BubbleTeaShopResource(mongoClient)
//
//        when:
...
//        def nearestShop = bubbleTeaShop.getNearest(latitude, longitude)
//
//        then:
...
//    }
    ...
    def 'should save all fields to the database when order is saved'() {
        ...
        setup:
        def mongoClient = new MongoClient()
        def database = mongoClient.getDB("bubbleTea")
        def collection = database.getCollection('Order')
        collection.drop()

        def bubbleTeaShop = new BubbleTeaShopResource(mongoClient)

        def drinkType = new DrinkType(family: 'Original', name: 'lychee')

        def order = new Order( size:'small', drinker:'koodle', drinkType: drinkType,
                selectedOptions: ['milk'])

        def shopId = 1

        when:
        Response response = bubbleTeaShop.createOrder(shopId, order);
        ...
        createdOrder['drinkType'].name == drinkType.name
        createdOrder['drinkType'].family == order.drinkType.family
        createdOrder['size'] == order.size
        createdOrder['drinker'] == order.drinker
        createdOrder['shopId'] == order.shopId
...
//
//        def bubbleTeaShop = new CoffeeShopResource(database)
